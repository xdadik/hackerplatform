          ...
          <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
              <div className="relative w-full sm:w-auto">
                ...
                <Input value={q} onChange={e=>{setQ(e.target.value); setPage(1)}} placeholder="Search challenges, tags..." className="pl-8 h-11 sm:h-8 w-full sm:w-[260px] bg-[var(--surface)] min-h-[44px] sm:min-h-0" />
              ...
              <Button variant="secondary" size="sm" className="h-11 sm:h-8 min-h-[44px] sm:min-h-0 shrink-0" onClick={()=>alert("Filters: use sidebar category/difficulty/status + search. All filters persist locally and combine.")}><Filter className="w-3.5 h-3.5 mr-1" /> Filters</Button>
            </div>
          </div>
        </FadeIn>
              ...
              <CardContent className="p-4 space-y-4">
                <div>
                  ...
                  <div className="space-y-1">
                    {categories.map(cat => (
                      <button key={cat} onClick={()=>{setActiveCat(cat); setPage(1)}} className={`w-full text-left px-2.5 py-2 sm:py-1.5 min-h-[36px] sm:min-h-0 rounded-[7px] text-[13px] flex items-center justify-between ${cat === activeCat ? "bg-[var(--text)] text-[var(--background)] font-[500]" : "text-[var(--text-2)] hover:bg-[var(--surface-2)] hover:text-[var(--text)]"}`}>
                        <span>{cat}</span>
                ...
                {(q || activeCat!=="All" || diffFilter.size!==2 || statusFilter.size!==1) && (
                  <Button variant="ghost" size="sm" className="w-full h-9 sm:h-7 min-h-[36px] sm:min-h-0 border" onClick={()=>{setQ(""); setActiveCat("All"); setDiffFilter(new Set(["Easy","Medium"])); setStatusFilter(new Set(["new"])); setPage(1)}}>Clear filters</Button>
                )}
              </CardContent>
            </Card>
          ...
          <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2">
                <div className="flex items-center gap-1 p-1 rounded-[8px] bg-[var(--surface-2)] border border-[var(--border)]">
                  {(["recommended","points","solves"] as const).map(s=> (
                    <button key={s} onClick={()=>setSort(s)} className={`px-2.5 py-2 sm:py-1 min-h-[36px] sm:min-h-0 rounded-[6px] text-[12px] font-[500] capitalize ${sort===s ? "bg-[var(--surface)] border border-[var(--border)] shadow-sm" : "text-[var(--text-2)]"}`}>{s}</button>
                  ))}
                </div>
            ...
            <Stagger className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
              ...
              {paged.length===0 && (
                ...
                <Button size="sm" className="mt-4 h-11 sm:h-8 min-h-[44px] sm:min-h-0 rounded-[8px] bg-[var(--text)] text-[var(--background)] hover:bg-zinc-800 dark:hover:bg-zinc-200" onClick={()=>{setQ(""); setActiveCat("All"); setDiffFilter(new Set(["Easy","Medium"])); setStatusFilter(new Set(["new"])); setPage(1)}}>Clear filters</Button>
              </Card></div>
              )}
            </Stagger>
            ...
            <div className="mt-6 flex items-center justify-between text-[12px] text-[var(--text-3)] border-t border-[var(--border)] pt-4">
              ...
              <div className="flex gap-1">
                <Button variant="secondary" size="sm" className="h-9 sm:h-7 min-h-[36px] sm:min-h-0 px-3" disabled={page<=1} onClick={()=>setPage(p=>Math.max(1,p-1))}>Previous</Button>
                <Button variant="secondary" size="sm" className="h-9 sm:h-7 min-h-[36px] sm:min-h-0 px-3" disabled={page>=totalPages} onClick={()=>setPage(p=>Math.min(totalPages,p+1))}>Next</Button>
              </div>
            </div>
          </div>
        </div>
