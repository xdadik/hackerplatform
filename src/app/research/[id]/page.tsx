...
import { ArrowLeft, Bookmark, MessageSquare, Eye, Clock, Award, Share2, FileText } from "lucide-react"
    ...
    <AppShell withSidebar>
      <div className="px-4 sm:px-6 lg:px-8 py-6 max-w-[860px]">
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="ml-auto hidden sm:flex items-center gap-1 text-[11px] text-[var(--text-3)]"><Eye className="w-3 h-3" />3,421 • <Bookmark className="w-3 h-3" />42 • <MessageSquare className="w-3 h-3" />18</span>
        </div>
      ...
      values   = [var.external_id]
        ...
        <Card className="mt-6">
          <CardContent className="p-5">
            <div className="text-[13px] font-semibold flex items-center gap-2"><MessageSquare className="w-3.5 h-3.5" /> Discussion • 18 comments</div>
            <div className="mt-4 space-y-3">
              {[
                ...
                <div key={c.user} className="flex gap-2.5 p-3 rounded-[10px] border border-[var(--border)] bg-[var(--surface-2)]">
              ...
              <div className="flex gap-2">
                <input placeholder="Add a comment — be technical, cite sources..." className="flex-1 h-9 rounded-[8px] border border-[var(--border)] bg-[var(--surface)] px-3 text-[13px] focus:outline-none focus:ring-2 focus:ring-[var(--accent)]" />
                <Button size="sm" className="h-9">Comment</Button>
              </div>
            </div>
          </CardContent>
        </Card>
