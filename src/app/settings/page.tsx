      ...
      <div className="flex min-h-[calc(100vh-56px)] bg-white dark:bg-[#0A0A0A] -m-6 lg:-m-8">
        ...
        <div className="w-[240px] shrink-0 border-r border-zinc-200 dark:border-zinc-800 bg-[#F9F9F7] dark:bg-[#0F0F0F] hidden lg:flex flex-col">
          ...
          <div className="p-3 border-t border-zinc-200 dark:border-zinc-800">
            <button onClick={()=>alert("Customize: Appearance, language, and notification preferences. Theme toggle is in Profile → Preferences.")} className="w-full flex items-center gap-2.5 px-2.5 py-2 rounded-[8px] text-[13px] text-zinc-600 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 text-left">
        ...
        <div className="flex-1 min-w-0 bg-white dark:bg-[#0A0A0A] p-6 lg:p-8 overflow-y-auto">
          {accountTab === "billing" ? (
            ...
            <div className="max-w-[720px]">
              ...
              <Link href="/settings/billing"><Button size="sm" className="mt-4 h-8">Go to Billing</Button></Link>
            </div>
          )}
        </div>
      </div>
    ...
    <div className="max-w-[720px]">
      ...
      <div className="mt-8 space-y-6">
        <div className="flex items-center justify-between py-4 border-b border-zinc-200 dark:border-zinc-800">
          ...
          <div className="w-10 h-10 rounded-full bg-[#7C3AED] flex items-center justify-center text-[13px] font-bold text-white">{(fullName||"NL").split(" ").map(s=>s[0]).join("").slice(0,2).toUpperCase()}</div>
        </div>

        <div className="flex items-center justify-between py-4 border-b border-zinc-200 dark:border-zinc-800">
          <span className="text-[14px] font-[500]">Full name</span>
          <Input value={fullName} onChange={e=>setFullName(e.target.value)} placeholder="Enter your full name" className="w-[320px] h-10 bg-white dark:bg-zinc-900 border-zinc-300 dark:border-zinc-700 text-[14px]" />
        </div>

        <div className="flex items-center justify-between py-4 border-b border-zinc-200 dark:border-zinc-800">
          <span className="text-[14px] font-[500]">What should Claude call you?</span>
          <Input value={preferred} onChange={e=>setPreferred(e.target.value)} placeholder="Enter preferred name" className="w-[320px] h-10 bg-white dark:bg-zinc-900 border-zinc-300 dark:border-zinc-700 text-[14px]" />
        </div>

        <div className="flex items-center justify-between py-3 border-b border-zinc-100 dark:border-zinc-800">
          <span className="text-[13px] font-[500]">What best describes your work?</span>
          <select value={work} onChange={e=>setWork(e.target.value)} className="text-[13px] border border-zinc-200 dark:border-zinc-700 rounded-[8px] px-3 py-1.5 bg-white dark:bg-zinc-900">
        ...
        <div className="pt-2">
          ...
          <div className="mt-4 flex items-center justify-between py-3 border-b border-zinc-100 dark:border-zinc-800">
            ...
            <div className="flex items-center gap-1 p-1 rounded-full bg-zinc-100 dark:bg-zinc-800">
              <button onClick={()=>setTheme("light")} className={`w-7 h-7 rounded-full flex items-center justify-center ${theme==="light" ? "bg-white shadow-sm text-amber-500" : "text-zinc-500"}`}><span className="text-[12px]">☀</span></button>
              <button onClick={()=>setTheme("dark")} className={`w-7 h-7 rounded-full flex items-center justify-center ${theme==="dark" ? "bg-white dark:bg-zinc-700 shadow-sm text-zinc-700" : "text-zinc-500"}`}><span className="text-[12px]">◐</span></button>
              <button onClick={toggle} className="w-7 h-7 rounded-full flex items-center justify-center text-zinc-500"><span className="text-[12px]">🖥</span></button>
            </div>
          </div>
          <div className="mt-4 flex gap-2">
            <Button size="sm" className="h-8" onClick={save}>{saved ? "✓ Saved" : "Save preferences"}</Button>
            <span className="text-[11px] text-zinc-500 self-center">Saved to localStorage (aegis_user, aegis_instructions)</span>
          </div>
        </div>
      </div>
    </div>
...
function AccountView() {
  ...
  const addDevice=()=>{
    const name=prompt("Device name (e.g., MacBook Pro)?")
    if(!name) return
    setDevices(prev=>[...prev, {id:Date.now().toString(), name, added:new Date().toLocaleDateString()}])
  }
    ...
    <div className="max-w-[720px]">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between py-3 border-b border-zinc-100 dark:border-zinc-800">
          ...
          <Button variant="secondary" size="sm" className="h-7 rounded-full border" onClick={handleLogoutAll}>Log out</Button>
        </div>
        <div className="flex items-center justify-between py-3 border-b border-zinc-100 dark:border-zinc-800">
          <span className="text-[13px]">Delete account</span>
          <Button variant="secondary" size="sm" className="h-7 rounded-full border" onClick={()=>{ if(confirm("Contact support to delete account?")) window.location.href="mailto:support@aegis.lab"}}>Contact support</Button>
        </div>
        <div className="flex items-center justify-between py-3 border-b border-zinc-100 dark:border-zinc-800">
      ...
      {devices.length===0 ? (
        <Card className="mt-3 border-dashed rounded-[12px] bg-zinc-50 dark:bg-zinc-900/50">
          <CardContent className="p-6 text-center">
            ...
            <Button size="sm" className="mt-4 h-8 rounded-[8px] bg-zinc-900 text-white hover:bg-zinc-800 dark:bg-white dark:text-zinc-900 dark:hover:bg-zinc-100" onClick={addDevice}>Add trusted device</Button>
          </CardContent>
        </Card>
      ) : (
        <div className="mt-3 space-y-2">
          {devices.map(d=>(
            <div key={d.id} className="flex items-center justify-between p-3 rounded-[10px] border border-[var(--border)] bg-[var(--surface)]">
              ...
              <Button variant="ghost" size="sm" className="h-7 border" onClick={()=>removeDevice(d.id)}>Remove</Button>
            </div>
          ))}
          <Button size="sm" className="mt-2 h-8" onClick={addDevice}>Add another device</Button>
        </div>
      )}
  ...
  return (
    <div className="max-w-[720px]">
      <div className="flex items-center gap-3 mb-6">
        ...
        <Link href="/settings/billing" className="ml-auto"><Button size="sm" className="h-8 rounded-full bg-zinc-900 text-white">Upgrade plan</Button></Link>
      </div>
      ...
      <Button variant="secondary" size="sm" className="mt-3 h-7" onClick={onBack}>Back to profile</Button>
    </div>
  )
}
