...
import { Search, Users, Crown, Shield, Trophy, Calendar, Activity, Settings, Plus } from "lucide-react"
    ...
    <AppShell withSidebar>
      <div className="px-4 sm:px-6 lg:px-8 py-6 sm:py-8 max-w-[1080px]">
        <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
          ...
          <Link href="/teams/new"><Button className="rounded-[8px]"><Plus className="w-4 h-4 mr-1" /> Create team</Button></Link>
        </div>

        <div className="flex items-center gap-2 mb-6">
          <div className="relative flex-1 max-w-[360px]">
            <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[var(--text-3)]" />
            <Input placeholder="Search teams, members..." className="pl-8 h-8 bg-[var(--surface)]" />
          </div>
          <Badge variant="outline" className="hidden sm:inline-flex rounded-full">3 teams • 2 owned</Badge>
        </div>
                ...
                <div className="flex flex-col lg:flex-row gap-6">
                  ...
                  <div className="lg:w-[340px] shrink-0 space-y-3">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button size="sm" variant="secondary" className="rounded-[8px]"><Settings className="w-3.5 h-3.5" /></Button>
                    </div>
          ...
          {teams.length === 0 && (
            <Card className="border-dashed rounded-[12px] bg-[var(--surface-2)]">
              <CardContent className="p-6 text-center">
                ...
                <Link href="/teams/new"><Button size="sm" className="mt-4 h-8 rounded-[8px] bg-[var(--text)] text-[var(--background)] hover:bg-zinc-800 dark:hover:bg-zinc-200">Create team</Button></Link>
              </CardContent>
            </Card>
          )}
        </div>
